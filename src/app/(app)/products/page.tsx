...
const page = () => {
  ...
  const handleDelete = async (e: any) => {
    ...
    try {
      setIsLoading(true)
      if(productToDelete){
        const data = {productToDelete}
        const response = await axios.delete('/api/products',{data});
        if(response.data.success){
          await fetchPorducts()
                    ...
                    {products.map((product: any) => (
                      <TableRow className='border-separate border-slate-400'>
                        ...
                        <TableCell className='flex gap-2 justify-center'>
                          ...
                          <Dialog>
                            <DialogTrigger>
                              <button onClick={()=>setProductToDelete(product._id)} className='btn-secondary gap-1 items-center text-xs font-medium inline-flex'>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
                            ...
                            <DialogContent>
                              <DialogHeader>
                                <DialogTitle className='my-4'>Are you absolutely sure?</DialogTitle>
                                <DialogDescription>
                              ...
                              <DialogFooter>
                                ...
                                <Button type='button' onClick={handleDelete} variant={'default'}>Delete</Button>
                              </DialogFooter>
                            </DialogContent>
                          </Dialog>
